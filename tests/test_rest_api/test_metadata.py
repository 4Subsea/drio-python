...
class Test_MetadataAPI(unittest.TestCase):
    ...
    def test_get_by_id(self, mock_token):
        mock_post = self.api._session.get

        mock_post.return_value = Mock()
        mock_post.return_value.text = u'{}'

        self.api.get_by_id(self.token, self.metadata_id)

        expected_uri = 'https://root/metadata/{}'.format(
            self.metadata_id)
    ...
    @patch('datareservoirio.rest_api.metadata.TokenAuth')
    def test_get_by_namespace_and_key(self, mock_token):
        mock_post = self.api._session.get

        mock_post.return_value = Mock()
        mock_post.return_value.text = u'{}'

        self.api.get(self.token, 'hello', 'world')
    ...
    @patch('datareservoirio.rest_api.metadata.TokenAuth')
    def test_put_by_id(self, mock_token):
        mock_post = self.api._session.put

        self.api.put_by_id(self.token, self.metadata_id, Norway='hei', Klingon='grabah')
    ...
    @patch('datareservoirio.rest_api.metadata.TokenAuth')
    def test_put_by_namespace_and_key(self, mock_token):
        mock_post = self.api._session.put

        self.api.put(self.token, 'hello', 'world', True, Norway='hei', Klingon='grabah')
    ...
    def test_namespaces_returns_list(self, mock_token):
        mock_response = Mock()
        mock_response.json = Mock(return_value=['system_1',
                                                'system_2'])
    ...
    def test_keys_returns_list(self, mock_token):
        mock_response = Mock()
        mock_response.json = Mock(return_value=['reservoir',
                                                'campaigns'])
